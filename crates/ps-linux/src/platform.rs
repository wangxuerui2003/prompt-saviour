...
impl CaptureHub {
    ...
    pub fn start(self: &Arc<Self>) -> anyhow::Result<()> {
        ...
        if !perms.at_spi {
            warn!("AT-SPI unavailable — GUI capture may be keystroke-only");
        }
...

        {
            ...
            thread::Builder::new()
                .name("ps-atspi-poll".into())
                .spawn(move || atspi_poll_loop(hub, poll_ms))
                .context("spawn atspi poll thread")?;
        }
        ...
        info!("linux capture hub started");
        Ok(())
    }
...
fn atspi_poll_loop(hub: Arc<CaptureHub>, poll_ms: u64) {
...
fn poll_atspi_once(hub: &CaptureHub) {
    ...
    match atspi_text::read_focused_text() {
        ...
        Ok(None) => {}
        Err(err) => error!(?err, "atspi poll failed"),
    }
}
